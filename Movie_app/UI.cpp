    ...
    while (true) {
        ...
        try {
            ...
            //add,"Title","Genre",Year,likes,"trailer"
                ...
                valid.validateMovie(Movie(tokens[1], tokens[2], std::stoi(tokens[3]), std::stoi(tokens[4]), tokens[5]));
...
void UI::startappUser(){
    ...
    while (true) {
        ...
        //watched "Title"
            ...
            controller.getWatchlist()->writeWatchlist();
        }
        else if(tokens[0] == "display") {
            controller.getWatchlist()->showWatchlist();
        }
    }
}
